...
const applicationSchema = yup.object().shape({
  ...
  supportingDocument: yup.mixed().required("Supporting Document is required"),
  ...
  domicileCertificate: yup.mixed().required("Domicile Certificate is required"),
      ...
      {({
            ...
            >
              {({ getRootProps, getInputProps }) => (
                ...
                >
                  <input {...getInputProps()} />
                  {!values[fieldName] ? (
                    <p>Add {label} Here (PDF or Image)</p>
                  ) : (
                    <FlexBetween>
                      <Typography>{values[fieldName].name}</Typography>
                      <EditOutlinedIcon />
                    </FlexBetween>
                  )}
                </Box>
        ...
        return (
          <form onSubmit={handleSubmit}>
            ...
            {/* Existing supporting document upload */}
            {renderDropzone("supportingDocument", "Supporting Document")}

            {/* New document upload fields */}
            {renderDropzone("signature", "Signature")}
            {renderDropzone("photo", "Photo")}
            {renderDropzone("aadharCard", "Aadhar Card")}
            {renderDropzone("tenthMarksheet", "10th Marksheet")}
            {renderDropzone("twelfthMarksheet", "12th Marksheet")}
            {renderDropzone("domicileCertificate", "Domicile Certificate")}
            {renderDropzone("casteCertificate", "Caste Certificate")}
